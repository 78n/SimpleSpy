...
function eventSelect(frame)
	if selected and selected.Log then
		if selected.Button then
			spawn(function()
				TweenService:Create(selected.Button, TweenInfo.new(0.5), { BackgroundColor3 = Color3.fromRGB(0, 0, 0) })
					:Play()
			end)
		end
		selected = nil
	...
	for i, v in next, t do
		...
		elseif rawtostring(i):match("^[%a_]+[%w_]*$") then
			ret = ret
				...
				.. rawtostring(count)
				.. " = "
				.. v2s(v, nil, nil, lower(rawtostring(i)) .. "_" .. rawtostring(count), true)
				.. "\n"
		else
			ret = ret
				.. "local "
...
local newnamecall = newcclosure(function(...)
	...
	if
		method
		and (method == "FireServer" or method == "fireServer" or method == "InvokeServer" or method == "invokeServer")
	then
		if typeof(...) == "Instance" then
			local remote = cloneref(...)
...
if not getgenv().SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		...
		bringBackOnResize()
		SimpleSpy3.Parent = (gethui and gethui())
			...
			if configs.advancedinfo then
				...
				info["advancedinfo"] = {
					...
					DebugId = {
						...
						CallingScriptDebugId = CallingScript and typeof(SourceScript) == "Instance" and OldDebugId(
							CallingScript
						) or "N/A",
			...
			if not DecompiledScripts[Source] then
				...
				xpcall(function()
					local decompiledsource =
						decompile(Source):gsub("-- Decompiled with the Synapse X Luau decompiler.", "")
...
if configs.supersecretdevtoggle then
	newButton("Load SSV2.2", function()
		...
		loadstring(
			game:HttpGet("https://raw.githubusercontent.com/exxtremestuffs/SimpleSpySource/master/SimpleSpy.lua")
		)()
	end)
	...
	newButton("SUPER SECRET BUTTON", function()
		return "You dont need a description you already know what it does"
	end, function()
